...
<?php
    ...
    public function domain_verified_field_callback()
    {
        ...
        $checked = isset($options['domain_verified']) && $options['domain_verified'] === 'on' ? 'checked' : '';

        echo '<div style="margin-bottom: 10px;">';
        echo '<label>';
        echo '<input type="checkbox" name="' . esc_attr(self::OPTION_NAME) . '[domain_verified]" ' . $checked . '> ';
    ...
    public function form_types_field_callback()
    {
        ...
        foreach ($form_types as $id => $label) {
            $checked = isset($options[$id]) && $options[$id] === 'on' ? 'checked' : '';
            echo '<div style="margin-bottom: 10px;">';
            echo '<label>';
            echo '<input type="checkbox" name="' . esc_attr(self::OPTION_NAME) . '[' . esc_attr($id) . ']" ' . $checked . '> ';
            echo esc_html($label);
            echo '</label>';
            echo '</div>';
        }
